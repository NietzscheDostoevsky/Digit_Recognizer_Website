...
function draw(ctx, x, y, size, isDown) {
    if (isDown) {   //to inform canvas user is about to draw
        ctx.beginPath();
...
(async function () {
    ...
    model = await tf.loadLayersModel("https://github.com/DietzscheNostoevsky/Digit_Recognizer_Website/blob/main/models/model.json")
